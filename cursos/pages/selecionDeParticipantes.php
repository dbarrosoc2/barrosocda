...
ORDER BY 
    ...
    sc.*, 
    (SELECT MAX(fechaEnvio) FROM certificados WHERE id_solicitud = sc.id_solicitud) AS fecha_certificado,
    (SELECT MAX(fechaEnvio) FROM mensajes WHERE id_solicitud = sc.id_solicitud) AS fecha_mensaje,
    (SELECT confirma FROM confirmacion WHERE id_solicitud = sc.id_solicitud ORDER BY fecha_confirmacion DESC LIMIT 1) AS ultima_confirmacion,
    m.fechaEnvio AS mensaje
FROM 
    solicitud_curso sc
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["envio_noti"])) {
    ...
    if (isset($_POST["seleccionar_participante"]) && (!empty($_POST["seleccionar_participante"]))) {
        ...
        // Redirigir a la página de procesamiento con los números de solicitud y el ID del curso
        header("Location: envioSMS.php?numeros_solicitud=" . urlencode(implode(',', $numeros_solicitud)) . "&nombre_corto_curso=" . urlencode($nombreCorto) . "&id_curso=" . urlencode($id_curso) . "&id_curso" . urldecode(implode(',', $nombre_solicitante)) . "&nombre_curso=" . urlencode($nombreCurso));
        exit();
    }
}
?>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th>
                                <div>
                                    Admitido <i class="fas fa-sort" style="margin-left: 5px;"></i>
                                </div>
                            </th>
                            <th>
                                <div>
                                    Categoría <i class="fas fa-sort" style="margin-left: 5px;"></i>
                                </div>
                            </th>
                            <th>
                                <div>
                                    Nombre <i class="fas fa-sort" style="margin-left: 5px;"></i>
                                </div>
                            </th>
                            <th>
                                <div>
                                    Apellidos <i class="fas fa-sort" style="margin-left: 5px;"></i>
                                </div>
                            </th>
                            <th>
                                <div>
                                    Teléfono<i class="fas fa-sort" style="margin-left: 5px;"></i>

                                </div>
                            </th>
                            <th>
                                <div>
                                    Email <i class="fas fa-sort" style="margin-left: 5px;"></i>
                                </div>
                            </th>
                            <th>
                                <div>
                                    Último <br> Envío <i class="fas fa-sort" style="margin-left: 5px;"></i>
                                </div>
                            </th>
                            <th>
                                <div>
                                    Confirma <i class="fas fa-sort" style="margin-left: 5px;"></i>
                                </div>
                            </th>
                            <th>
                                <div>
                                    F. Env <br>Certificado <i class="fas fa-sort" style="margin-left: 5px;"></i>
                                </div>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        $consulta = "SELECT fechaEvio from certificados WHERE id_solicitud = ";
                        $alternate = true;
                        foreach ($cursos as $curso): ?>
                            <tr class="<?php echo $alternate ? 'table-primary' : 'table-secondary'; ?>">
                                ...
                                <td>
                                    ...
                                    <?php if (!empty($curso['fecha_mensaje'])): ?>
                                        <?php echo $curso['fecha_mensaje']; ?>
                                        <div>
                                ...
                                <td>
                                    ...
                                    if (isset($curso['fecha_certificado'])) {
                                        $fechaFormateada = date("d/m/y", strtotime($curso['fecha_certificado']));
                                        $horaFormateada = date("H:i", strtotime($curso['fecha_certificado']));
