...
if (!empty($id_solicitud)) {
    ...
    $nombreLargo = $_GET['nombre_curso'];
    if (strlen($nombreLargo) > 60) {
        $nombreRecortado = substr($nombreLargo, 0, 57) . '...'; // Obtiene los primeros 57 caracteres y añade "..."
    } else {
        $nombreRecortado = $nombreLargo; // Si la cadena es menor o igual a 60 caracteres, se mantiene igual
    }
    $curso = !empty($_GET['nombre_corto_curso']) ? $_GET['nombre_corto_curso'] : $nombreRecortado;
