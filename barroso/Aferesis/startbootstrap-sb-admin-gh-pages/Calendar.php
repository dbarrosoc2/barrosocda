...
<body>
    ...
    function generarCalendario($mes, $anio, $citas)
    {
        ...
        for ($dia = 1; $dia <= $numDias; $dia++) {
            ...
            if (isset($citasProgramadas[$dia])) {
                echo "<td class='no-disponible' id='$idDia'>$dia</td>"; // Marcar como no disponible si hay cita programada
            } else {
                echo "<td class='disponible abrir-ventana' id='$idDia'>$dia</td>"; // Agregar la clase "abrir-ventana"
            }
...
<script>
    ...
    celdasDia.forEach(function (celda) {
        celda.addEventListener('click', function () {
            ...
            var diaSeleccionado = this.textContent;
            // Abrir una nueva ventana con los horarios disponibles para el día seleccionado
            var nuevaVentana = window.open('horarios_disponibles.php?dia=' + diaSeleccionado, '_blank', 'width=600,height=400');
        });
    });
</script>
