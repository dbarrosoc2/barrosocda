        ...
        <div class="container-fluid px-4">
            ...
            function generarCalendario($mes, $anio, $citas)
            {
                ...
                foreach ($diasSemana as $dia) {
                    if ($dia === 'S' || $dia === 'D') {
                        echo "<th class='amarillo-claro'>$dia</th>"; // Sábados y domingos en amarillo claro
                    } else {
                        echo "<th>$dia</th>"; // Resto de los días
                    }
                }
                echo '</tr></thead>';
                echo '<tbody><tr>';
                ...
                for ($dia = 1; $dia <= $numDias; $dia++) {
                    ...
                    if ($fecha < new DateTime('today')) {
                        ...
                        if ($fecha->format('N') >= 6) {
                            echo "<td class='amarillo-claro abrir-ventana' id='$idDia' data-dia='$dia' data-mes='$mes' data-anio='$anio'>$dia</td>"; // Sábados y domingos en amarillo claro
                        } else {
                            echo "<td class='abrir-ventana' id='$idDia' data-dia='$dia' data-mes='$mes' data-anio='$anio'>$dia</td>"; // Resto de los días
                        }
                    }
...
<script>
    ...
    celdasDia.forEach(function (celda) {
        celda.addEventListener('click', function () {
            ...
            var nuevaVentana = window.open('horarios_disponibles.php?dia=' + diaSeleccionado + '&mes=' + mesSeleccionado + '&anio=' + anioSeleccionado, '_blank', 'width=600,height=400');
        });
    });
</script>
<script>
    // Obtener todas las celdas de la tabla
    var celdas = document.querySelectorAll('.table-bordered td');

    // Iterar sobre cada celda y aplicar el estilo a los sábados y domingos
    celdas.forEach(function (celda) {
        var contenido = celda.textContent;
        if (contenido === 'S' || contenido === 'D') {
            celda.classList.add('amarillo-claro');
        }
    });
</script>
