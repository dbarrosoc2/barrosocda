...
class Usuarios
{
    ...
    public function cambiarRolUsuario($id_usuario, $rol)
    {
        ...
        try {
            ...
            $query = "UPDATE usuario SET id_rol = :id_rol WHERE dni = :id_usuario";
    ...
    public function listarUsuariosSolicitantes()
    {
        try {
            ...
            $query = "SELECT * FROM personal WHERE id_rol IS NULL OR id_rol = ''";
            ...
            foreach ($usuarios as $usuario) {
                ...
                $output .= '<tr class="' . $row_class . ' clickable-row" data-id="' . $usuario['cod_empleado'] . '">';
                $output .= '<td>' . $usuario['dni'] . '</td>';
                $output .= '<td>' . $usuario['nombre'] . '</td>';
                $output .= '<td>' . $usuario['apellidos'] . '</td>';
                $output .= '</tr>';
    ...
    public function ObtenerUsuarioPorId($id_usuario)
    {
        try {
            ...
            $query = "SELECT * FROM personal WHERE dni = :id_usuario";
    ...
    public function listarUsuarios()
    {
        try {
            ...
            $query = "SELECT personal.*, rol.descripcion_rol AS descripcion_rol 
                  FROM personal 
                  LEFT JOIN rol ON personal.id_rol = rol.id_rol";
            ...
            foreach ($usuarios as $usuario) {
                ...
                $id_usuario = substr($usuario['dni'], -5);
                $output .= '<td>' . $id_usuario . '</td>';
                $output .= '<td><a href="perfil.php?id=' . $usuario['dni'] . '">' . $usuario['nombre'] . '</a></td>';
    ...
    public function crearUsuario($id_usuario, $nombre, $apellido1, $apellido2)
    {
        ...
        // Preparar la consulta SQL
        $query = "INSERT INTO usuario (dni, nombre, apellido1, apellido2)
